...
async def test_concurrent_performance(execution_loop):
    ...
    async def run_task(task_id: int):
        ...
        try:
            ...
            result = await asyncio.wait_for(
                execution_loop.run(task), timeout=STRESS_TEST_CONFIG["task_timeout"]
            )
    ...
    if metrics.response_times:
        assert (
            summary["response_time"]["avg_ms"] <= STRESS_TEST_CONFIG["response_time_threshold_ms"]
...
async def test_error_recovery(execution_loop):
    ...
    try:
        result = await asyncio.wait_for(
            execution_loop.run(normal_task), timeout=STRESS_TEST_CONFIG["task_timeout"]
        )
    ...
    try:
        final_result = await asyncio.wait_for(
            execution_loop.run(final_task), timeout=STRESS_TEST_CONFIG["task_timeout"]
        )
    ...
    for concurrency in concurrency_levels:
        ...
        try:
            success, duration = await asyncio.wait_for(
                run_batch(), timeout=STRESS_TEST_CONFIG["task_timeout"] * 2
            )
