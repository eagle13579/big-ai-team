    ...
    async def arun(self, prompt):
        ...
        if self.step == 1:
            return MockRunResponse(
                content="I tried to divide 10 by 0, but it failed with an error: Cannot divide by zero"
            )
        # 模拟第二次执行会成功
        elif self.step == 2:
            return MockRunResponse(
                content="Final Answer: 5.0"
            )
        # 其他情况
        else:
            return MockRunResponse(
...
class AgenticLoop:
    ...
    async def run_until_complete(self, task: str):
        step = 0
        current_prompt = task
    ...
    def _generate_correction_prompt(self, response):
        # 让 Agent 反思为什么失败
        return f"Previous attempt failed. Please adjust your strategy and try again."


async def test_loop_self_heal():
    ...
    assert "5.0" in result.content or "5" in result.content, "Response should contain the correct result (5)"
