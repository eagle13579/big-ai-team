...
def test_basic_features():
    ...
    for i, model in enumerate(cheapest[:5]):
        cost_info = (
            "🆓 免费"
            if model.is_free
            else f"${(model.cost_per_1k_tokens_input + model.cost_per_1k_tokens_output) / 2:.4f}/1K tokens"
        )
    ...
    for model in all_policies:
        if model.provider in domestic_providers:
            cost_info = (
                "🆓 免费"
                if model.is_free
                else f"${(model.cost_per_1k_tokens_input + model.cost_per_1k_tokens_output) / 2:.4f}/1K"
            )
