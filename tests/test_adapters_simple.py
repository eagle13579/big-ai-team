...
async def test_mock_llm_adapter():
    ...
    adapter = AdapterFactory.create_llm_adapter(
        platform="mock_llm", name="test_mock_llm", api_key="test_key"
    )
    ...
    result = await adapter.execute(
        operation="generate", params={"prompt": "Hello, world!"}, context=context
    )
...
async def test_memory_cache_adapter():
    ...
    print("\n测试内存缓存适配器...")
    # 创建适配器
    adapter = AdapterFactory.create_cache_adapter(platform="memory_cache", name="test_memory_cache")
    ...
    # 测试获取缓存
    result = await adapter.execute(operation="get", params={"key": "test_key"}, context=context)
    assert result["value"] == "test_value"
    print(f"✅ 获取缓存测试通过，值: {result['value']}")

    # 测试删除缓存
    delete_result = await adapter.execute(
        operation="delete", params={"key": "test_key"}, context=context
    )
...
async def test_local_storage_adapter():
    ...
    adapter = AdapterFactory.create_storage_adapter(
        platform="local_storage", name="test_local_storage", base_path="./test_storage"
    )
    ...
    # 测试读取文件
    result = await adapter.execute(
        operation="read", params={"path": "test_file.txt"}, context=context
    )
    assert result["content"] == "Hello, storage!"
    print(f"✅ 读取文件测试通过，内容: {result['content']}")

    # 测试列出文件
    list_result = await adapter.execute(operation="list", params={"directory": ""}, context=context)
    assert "test_file.txt" in list_result["files"]
    print(f"✅ 列出文件测试通过，文件列表: {list_result['files']}")

    # 测试删除文件
    delete_result = await adapter.execute(
        operation="delete", params={"path": "test_file.txt"}, context=context
    )
    assert delete_result["deleted"]
    print("✅ 删除文件成功")
