...
async def demo_url_detection():
    ...
    for url, expected in test_urls:
        ...
        if channel:
            status = (
                "✅"
                if channel.name == expected.lower() or expected.lower() in channel.name
                else "⚠️"
            )
            print(f"{status} {url}")
...
async def demo_skill_execution():
    ...
    if result["status"] == "success":
        ...
        print(f"   ⚠️ {result['observation']['message']}")
        print("   💡 提示: 确保已安装 gh CLI (https://cli.github.com)")
...
async def demo_workflow_integration():
    ...
    workflow_steps = [
        ...
        (
            "搜索 GitHub 项目",
            "search_github_repos",
            {"query": "AI Agent framework", "language": "python", "limit": 5},
        ),
    ...
    for step_name, action, params in workflow_steps:
        ...
        if result["status"] == "success":
            print("   ✅ 完成")
            results.append(
                {"step": step_name, "status": "success", "data": result["observation"]["data"]}
            )
        else:
