...
class CalculatorArgsSchema(BaseModel):
    ...
    operation: str = Field(
        ...,
        description="操作类型: add, subtract, multiply, divide, power, sqrt, sin, cos, tan, log, log10, exp, abs",
    )
...
class CalculatorTool(BaseSkill):
    ...
    def execute(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            # 验证参数类型
            if not isinstance(args.get("a"), (int, float)) or not isinstance(
                args.get("b"), (int, float)
            ):
                return {
                    "status": "error",
                    "observation": {
