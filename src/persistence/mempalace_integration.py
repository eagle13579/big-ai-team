...
class MemPalaceIntegration:
            ...
            try:
                with open(self.graph_file, "r", encoding="utf-8") as f:
                    ...
                    for edge in graph_data.get("edges", []):
                        self.knowledge_graph.add_edge(edge["source"], edge["target"], **edge.get("attributes", {}))
            except Exception as e:
    ...
    def search(self, query: str, limit: int = 5, context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """搜索记忆（支持上下文感知和向量搜索）"""
        try:
            # 上下文感知搜索
            if context:
    ...
    def _vector_search(self, query: str, limit: int = 5, context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """向量搜索"""
        try:
    ...
    def _merge_search_results(self, traditional_results: List[Dict[str, Any]], vector_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """合并搜索结果"""
        # 使用集合去重
        memory_ids = set()
        merged_results = []
    ...
    def add_memory(self, content: str or Dict[str, Any], context: Dict[str, Any] = None, tags: List[str] = None) -> Dict[str, Any]:
    ...
    def _update_knowledge_graph(self, memory_id: str, content: str, context: Dict[str, Any], tags: List[str]):
        """更新知识图谱"""
        try:
            # 添加记忆节点
            self.knowledge_graph.add_node(memory_id, 
                                         type="memory",
            ...
            for keyword in keywords[:5]:  # 取前5个关键词
                ...
                if not self.knowledge_graph.has_node(keyword_id):
                    self.knowledge_graph.add_node(keyword_id, 
                                                 ...
                                                 name=keyword)
                # 添加记忆与关键词的边
                self.knowledge_graph.add_edge(memory_id, keyword_id, 
                                            type="contains",
                                            weight=0.8)
            ...
            if context:
                ...
                if "user" in context:
                    ...
                    if not self.knowledge_graph.has_node(user_id):
                        self.knowledge_graph.add_node(user_id, 
                                                     type="user",
                                                     name=context['user'])
                    self.knowledge_graph.add_edge(memory_id, user_id, 
                                                type="related_to",
                                                weight=0.9)
                ...
                if "project" in context:
                    ...
                    self.knowledge_graph.add_edge(memory_id, project_id, 
                                                type="part_of",
                                                weight=0.95)
            ...
            for related_id, similarity in related_memories:
                if related_id != memory_id:
                    self.knowledge_graph.add_edge(memory_id, related_id, 
                                                type="related",
                                                weight=similarity)
        except Exception as e:
    ...
    def _find_related_memories(self, content: str, limit: int = 3) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            # 排序并返回前N个
            sorted_similarities = sorted(similarities.items(), key=lambda x: x[1], reverse=True)[:limit]
    ...
    def assess_memory_quality(self, memory_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            # 计算综合质量分数
            quality_score = (accuracy * 0.3 + completeness * 0.25 + relevance * 0.25 + confidence * 0.2)
    ...
    def update_memory(self, memory_id: str, content: str = None, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """更新记忆（支持知识图谱和向量嵌入更新）"""
        try:
            # 查找记忆
            found = False
            for tier, memories in self.memory_tiers.items():
                if memory_id in memories:
                    ...
                    # 重新评估质量
                    self._evaluate_memory_quality(memory_id, memory["original_content"], memory["context"])
        ...
        try:
            ...
            if len(related_memories) < limit and self.embeddings.get(memory_id):
                ...
                for related_id, similarity in vector_related:
                    if related_id != memory_id and not any(m.get("id") == related_id for m in related_memories):
    ...
    def get_memory_recommendations(self, context: Dict[str, Any], limit: int = 5) -> List[Dict[str, Any]]:
        """智能记忆推荐"""
        try:
            # 提取上下文关键词
            ...
            def get_score(memory):
                return (memory.get("quality_score", 0) * 0.4 +
    ...
    def get_knowledge_graph_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "memory_nodes": len([n for n in self.knowledge_graph.nodes() if n.startswith("memory_")]),
                "keyword_nodes": len([n for n in self.knowledge_graph.nodes() if n.startswith("keyword_")]),
                "user_nodes": len([n for n in self.knowledge_graph.nodes() if n.startswith("user_")]),
                "project_nodes": len([n for n in self.knowledge_graph.nodes() if n.startswith("project_")]),
                "average_degree": sum(d for n, d in self.knowledge_graph.degree()) / self.knowledge_graph.number_of_nodes() if self.knowledge_graph.number_of_nodes() > 0 else 0
            }
            return stats
        except Exception as e:
    ...
    def get_clustering_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "average_cluster_size": sum(len(v) for v in self.clusters.values()) / len(self.clusters) if self.clusters else 0
            }
            return stats
        except Exception as e:
    ...
    def _sort_by_context_relevance(self, results: List[Dict[str, Any]], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """按上下文相关性排序"""
    ...
    def _calculate_context_relevance(self, result: Dict[str, Any], context: Dict[str, Any]) -> float:
        """计算与上下文的相关性分数"""
        score = 0.0
        ...
        if "time" in context:
            try:
                ...
                result_time = datetime.fromisoformat(result.get("created_at", datetime.now().isoformat()))
