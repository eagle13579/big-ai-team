...
class ModelPolicyTracker:
    ...
    def estimate_cost(self, model_name: str, input_tokens: int, output_tokens: int) -> CostEstimate:
        """
        预估使用成本
        """
            ...
            "top_5_cheapest_paid": [
                {
                    "name": m.model_name,
                    "provider": m.provider,
                    "avg_cost_per_1k": (m.cost_per_1k_tokens_input + m.cost_per_1k_tokens_output)
                    / 2,
                    "notes": m.notes,
                }
