...
class SkillRegistryManager:
    ...
    def register_skill(self, manifest: SkillManifest) -> SkillRegistry:
        ...
        # 检查技能是否已存在
        existing_skill = (
            self.db.query(SkillRegistry).filter(SkillRegistry.name == manifest.name).first()
        )
    ...
    def get_skill(self, name: str) -> SkillRegistry | None:
        ...
        return (
            ...
            .filter(SkillRegistry.name == name, SkillRegistry.is_active)
            .first()
        )

    def list_skills(self) -> list[SkillRegistry]:
        """列出所有激活的技能"""
        return self.db.query(SkillRegistry).filter(SkillRegistry.is_active).all()
