...
class TeamMode:
    ...
    def _determine_assignee(self, task_description: str, role_mapping: Dict[str, List[str]], available_roles: List[str]) -> str:
        """确定任务负责人"""
        for role, keywords in role_mapping.items():
            if role in available_roles and any(keyword in task_description.lower() for keyword in keywords):
                return role
