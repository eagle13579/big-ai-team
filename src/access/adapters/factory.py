...
class AdapterFactory:
    ...
    def create_adapter(platform: str, name: str, config: dict[str, Any]) -> BaseAdapter:
        ...
        Returns:
            BaseAdapter: 适配器实例
        """
        adapter_config = AdapterConfig(name=name, platform=platform, config=config)
    ...
    def create_database_adapter(
        platform: str, name: str, connection_string: str, **kwargs
    ) -> BaseAdapter:
        """创建数据库适配器
        ...
        """
        config = {"connection_string": connection_string, **kwargs}
        return AdapterFactory.create_adapter(platform, name, config)
