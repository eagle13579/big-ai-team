...
class BaseAdapter(ABC, Generic[T]):
    ...
    @abstractmethod
    async def execute(self, operation: str, params: Dict[str, Any], context: Optional[AdapterContext] = None) -> T:
        """执行操作"""
        pass
    ...
    async def health_check(self, context: Optional[AdapterContext] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Health check failed for {self.platform} adapter: {self.name}, error: {str(e)}")
            return {
                "status": "unhealthy",
                "platform": self.platform,
                "name": self.name,
